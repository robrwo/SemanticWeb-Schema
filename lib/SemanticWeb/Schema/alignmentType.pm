use utf8;

package SemanticWeb::Schema::alignmentType;

# ABSTRACT: A category of alignment between the learning resource and the framework node

use v5.14;
use Moo;

extends qw/ SemanticWeb::Schema /;


use MooX::JSON_LD 'alignmentType';
use Ref::Util qw/ is_plain_hashref /;
# RECOMMEND PREREQ: Ref::Util::XS

use namespace::autoclean;

our $VERSION = 'v24.0.0';

=encoding utf8

=head1 DESCRIPTION

A category of alignment between the learning resource and the framework
node. Recommended values include: 'requires', 'textComplexity',
'readingLevel', and 'educationalSubject'.




=cut


=head1 SEE ALSO



L<SemanticWeb::Schema>

=cut

1;
