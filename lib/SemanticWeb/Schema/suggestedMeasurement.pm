use utf8;

package SemanticWeb::Schema::suggestedMeasurement;

# ABSTRACT: A suggested range of body measurements for the intended audience or person

use v5.14;
use Moo;

extends qw/ SemanticWeb::Schema /;


use MooX::JSON_LD 'suggestedMeasurement';
use Ref::Util qw/ is_plain_hashref /;
# RECOMMEND PREREQ: Ref::Util::XS

use namespace::autoclean;

our $VERSION = 'v24.0.0';

=encoding utf8

=head1 DESCRIPTION

A suggested range of body measurements for the intended audience or person,
for example inseam between 32 and 34 inches or height between 170 and 190
cm. Typically found on a size chart for wearable products.




=cut


=head1 SEE ALSO



L<SemanticWeb::Schema>

=cut

1;
